...
本地基础设施已就绪：
- ✅ `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/poisemath.com` — content repo（vault + .gitignore + GitHub Action + 示例 markdown）
- ✅ `~/code/poisemath-site` — Quartz 4 + sync 脚本 + 配置
...
**content repo（vault）**：
```bash
cd "$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/poisemath.com"
gh repo create poisemath-content --private --source=. --remote=origin --push
```

**site repo（Quartz）**：
```bash
cd "$HOME/code/poisemath-site"
gh repo create poisemath-site --public --source=. --remote=origin --push
```
...
授权 GitHub → 选 `poisemath-site` repo → 配置：

| 字段 | 值 |
|------|-----|
| Project name | `poiseacademy-site` |
...
- `NODE_VERSION` = `22`
- `CONTENT_REPO` = `https://github.com/<你的用户名>/poisemath-content.git`

> 如果 content repo 私有，把 URL 改成带 PAT 的形式：`https://<USER>:<PAT>@github.com/<USER>/poisemath-content.git`，并在 Cloudflare 把 PAT 存为 secret env var。

点 **Save and Deploy**，第一次构建约 2 分钟。成功后会得到 `poiseacademy-site.pages.dev` 临时域名。
...
## 5. 绑定自定义域名 poisemath.com

Cloudflare Pages 项目 → Custom domains → Set up a custom domain → 输入 `poisemath.com`。

如果域名 NS 已经托管在 Cloudflare：自动添加 CNAME，几秒生效。

如果在别的注册商：按 Cloudflare 提示加 CNAME 记录指向 `poiseacademy-site.pages.dev`，并把根域 ALIAS / CNAME flattening 设好。
...
1. 打开 Obsidian → 切换到 `poisemath.com` vault
...
**改主题/字体**：编辑 `~/code/poisemath-site/quartz.config.ts` → 推 site repo → Cloudflare 自动重建。
